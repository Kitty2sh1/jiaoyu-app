<template>
	<view class="bind-phone">
		...
		<view class="bindInput">
			<h2>绑定手机号</h2>
			<user-input :formList="formList" v-model.sync="form" :sendOutText="bindText" @getCode="getCode">
			</user-input>
			<!-- 绑定按钮 -->
			<view class="btn">
				<button class="button" @click="handleBinding">绑 定</button>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async handlePhone() {
				try {
					const res = await loginApi.getPhone(this.form)
